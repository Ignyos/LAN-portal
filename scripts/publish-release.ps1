...
function Get-NextPatchVersion {
    ...
    if ($CurrentVersion -notmatch '^(?<major>0|[1-9]\d*)\.(?<minor>0|[1-9]\d*)\.(?<patch>0|[1-9]\d*)') {
        Exit-WithError -Code $ExitCodes.VersionValidationFailed -Message "Current version '$CurrentVersion' is not valid for patch increment."
    }
...
$targetVersion = if ([string]::IsNullOrWhiteSpace($enteredVersion)) { $defaultVersion } else { $enteredVersion.Trim() }
